SELECT CASE t.subcateg WHEN -1 THEN ca.categname ELSE ca.categname || ':' || sc.subcategname END as category,
  ...
  FROM (
           SELECT strftime('%m', TRANSDATE) AS month,
                  ...
                  IFNULL(CH.CURRVALUE, CF.BASECONVRATE) AS CURRVALUE,
       ...
       LEFT JOIN
       subcategory_v1 sc ON sc.categid = t.categ AND 
                            sc.subcategid = t.subcateg
 GROUP BY category
